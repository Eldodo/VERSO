...
public class PackageRenderHierarchicalVisitor implements IRepresentationVisitor{

	int packageLevel = 0 ;
	
	public PackageRenderHierarchicalVisitor(int level)
	{
		this.packageLevel = level;
	}
	
	public void visit(PackageRepresentation p) {
		if (p.getPackageLevel() <= packageLevel)
		{
			p.setRender(true);
		}
		else
