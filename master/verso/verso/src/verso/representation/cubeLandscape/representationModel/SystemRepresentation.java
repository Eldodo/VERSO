			...
			if (currLinksTable.get(startNode) != null) {
				...
				for (NodeRepresentation endNode : currLinksTable.get(startNode).keySet()) {
					if (this.nodesDisplayed.contains(endNode)) {
						...
						//****************************************************************************************
						//RéGLER LE PROBLéME DES LIENS EDGE BUNDLES QUI TOUCHENT PAS AUX CLASSES!!!! (ET PACKAGES,
						//MAIS éA C'EST SéREMENT JUSTE LE NOEUD DU PACKAGE QUI EST TROP HAUT).
...
//						}
	...
	protected LinkedList<NodeRepresentation> getNodesPath(NodeRepresentation startNode, NodeRepresentation endNode, LinkedList<Integer> lca, float outRatio) {
		...
		//é garder seulement si on ne merge pas les noeuds trop proche
		...
		if (newStartNode != null) {				
			...
			//Sert é enlever le "double" du noeud sur le package dans le path de noeud
	...
	*/
	public void updateNodesAndLinksDisplayed(String name) {
		...
		System.out.println("("+name+") Temps pour créer les liens: " + (afterNbreMS - beforeNbreMS));
	}
	...
	public ArrayList<EntityRepresentation> getEntityRepFromString(ArrayList<String> list){
		...
		if(!list.isEmpty())System.out.println("Liste non trouvé:");
		for(String path : list) {
		...
		if (currSortedLinks != null) {
			...
			for (IPickable startElement : startElements) {	
				...
				if (node != null) {
					...
					//é déplacer dans SceneLandscape ??? (car il y une différence entre isFILTERED et isSELECTED)
	...
	//Ajouter une fonction pour vider la liste de packages du systéme.
	public void clearPackages() {
		this.packages.clear();
	}
	
	
	//Fonctions pour trouver la taille (en y) de la plus grande boétes (classe).
	/*
	public float findMinY() {
