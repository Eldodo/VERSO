...
* et mÈlange modele et representation e.g le boolÈen displayLink est prÈsent dans les deux classes mais seul celui de la classes SystemRepresentation semble
* avoir un effet.
* - Il faut enlever les composants graphiques provenant de swing
* - le nommage est assez confus, le sens des mots n'est pas toujours le mÍme
* - le handler pour un clique souris dans SceneLandscape fait 400 lignes, surement moyen de le dÈcouper
* - parfois les interfaces sont construites "‡ la main", parfois avec un XML, parfois un mÈlange des deux... il faut homogÈnÈiser...
* ==> pour refactorer tout Áa, le plus simple serait sans doute de modeler et coder une base saine uniquement en JavaFX, et d'y 
* rÈimplanter les algos utilisÈs dans cette version, et les autres Èventuellement.
*/
...
//		treemapSysRep.linkAndNodeLayoutin(tempTreemapNodeLayout);
	...
	private static void linkin(SystemRepresentation sysRep, 
		...
		System.out.println("("+sysRep.getPackageRepresentationClass().getSimpleName()+") Temps pour cr√©er les tables de liens (en ms): " + (afterNbreMS - beforeNbreMS));
...
//				System.out.println(ioe.toString());
	...
	private static ColiseumPackageRepresentation coliseumLayouting(float bordersSize, SystemRepresentation coliseumSysRep) {
		...
		//Calcul du layout pour la visualisation de type "Colis√©"
		//******************************		
		...
		if (!coliseumBestLayout) {
			...
			try {
				...
				System.out.println("Rayon intern du colis√©e: " + pRootColiseum.getLayoutInternRadius());
			}
			catch (IOException ioe) {
				System.out.println(ioe.toString());
			}
...
//							currList.add((ColiseumPackageRepresentation)allPackages.get(solutionElements[i]));
	...
	private void putOnUI(Stage primaryStage, HashMap<String, SystemRepresentation> sysreps, String chosenSysRep, SceneLandscape sc) {
		try{
            ...
            secondColor.setOnAction(new EventHandler<ActionEvent>() {
				...
				public void handle(ActionEvent arg0) {
					for(String s : sysreps.keySet()) {
						sysreps.get(s).setSecondColor(secondColor.getValue());
						sc.updateHeatMap();
						sc.redisplay();
					}
				}
	...
	//		linksTypeControlPanels.put("Liens hi√©rarchiques", new HierarchicalLinksControlPanel(sc, chosenToStartApp, hierarchicalLinksMeshes));
