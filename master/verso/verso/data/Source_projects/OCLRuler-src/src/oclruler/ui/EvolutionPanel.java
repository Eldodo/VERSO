...
public class EvolutionPanel extends JPanel {
	private static final long serialVersionUID = 6692136674178440549L;
	...
	public EvolutionPanel(String metamodelName, String timeStamp) {
		super(new BorderLayout());
		
		String chartTitle = metamodelName+" - "+Evolutioner.GENERATION_MAX+"g x "+Population.NB_ENTITIES_IN_POP+"prg ";
	...
	private XYDataset createDataset() {
		...
		int objConsidered = FitnessVector.OBJECTIVES_CONSIDERED - (Config.TFIDF.isObjective()?1:0);
		
		XYSeries s;
		for (int i = 0; i < objConsidered; i++) {
		...
		if(Config.TFIDF.isObjective()) {
			s = new XYSeries("tfidf");
			seriesObjectives.add(s);
			dataset.addSeries(s);
			if(Ui.PRINT_FIRSTS){
				XYSeries sMax = new XYSeries("tfidf" +" first");
				seriesObjectives.add(sMax);
				dataset.addSeries(sMax);
			}
		}
...
//		renderer.setSeriesStroke(2, new BasicStroke(2.0f));
		...
		int nbCurbs = Ui.PRINT_FIRSTS? FitnessVector.OBJECTIVES_CONSIDERED*2 : FitnessVector.OBJECTIVES_CONSIDERED;
