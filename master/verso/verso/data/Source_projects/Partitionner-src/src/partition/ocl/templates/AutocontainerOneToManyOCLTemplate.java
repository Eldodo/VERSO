...
public class AutocontainerOneToManyOCLTemplate extends OCLTemplate {
	public final static Logger LOGGER = Logger.getLogger(AutocontainerOneToManyOCLTemplate.class.getName());
				...
				for (EReference erefContainedIn : ((EClass)eo).getEAllReferences()) {
					if(!erefContainedIn.isMany()){
						for (EReference erefContents : ((EClass)eo).getEAllReferences()) {
							if(erefContents.isMany() 
									&& erefContainedIn != erefContents 
									&& ((EClass)eo) == erefContents.getEReferenceType()){
	...
	private OCLMatch buildOCLMatch(EClass context, EReference erContainedIn, EReference erContents) {
		...
		String expr =  "self."+contents+"->forAll(a : "+contentsEClass+"| "
				+ "a."+containedIn+" = self)";
		OCLMatch res = new OCLMatch("context "+autoContainerEClass+" inv AutocontainerOneToMany: ", expr, context);
	...
	public String toString() {
		return "AutocontainerOneToManyOCLTemplate";
	}
}
