...
public class CollectionsSameSizeOCLTemplate extends OCLTemplate {
	public final static Logger LOGGER = Logger.getLogger(CollectionsSameSizeOCLTemplate.class.getName());
			...
			if(eo instanceof EClass){
				for (EStructuralFeature collectionOne : ((EClass)eo).getEAllStructuralFeatures()) {
					if(collectionOne.isMany()){
						for (EStructuralFeature collectionTwo : ((EClass)eo).getEAllStructuralFeatures()) {
							if(collectionOne != collectionTwo && collectionTwo.isMany()){
								OCLMatch oclinv = buildOCLInv(((EClass)eo), collectionOne, collectionTwo);
								matches.add(oclinv);
	...
	private OCLMatch buildOCLInv(EClass clA,  EStructuralFeature collectionOneRef, EStructuralFeature collectionTwoRef) {
		String classA = clA.getName();
		String collectionOne = collectionOneRef.getName();
		String collectionTwo = collectionTwoRef.getName();
		
		String expr = collectionOne+"->size() = "+collectionTwo+"->size()";
		OCLMatch res = new OCLMatch("context "+classA+" inv CollectionsSameSize: ", expr, clA);
	...
	public String toString() {
		return "CollectionsSameSizeOCLTemplate";
	}
}
