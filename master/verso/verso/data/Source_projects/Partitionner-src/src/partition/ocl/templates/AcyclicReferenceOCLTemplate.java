 ...
 *
 */
public class AcyclicReferenceOCLTemplate extends OCLTemplate {
	public final static Logger LOGGER = Logger.getLogger(AcyclicReferenceOCLTemplate.class.getName());
	...
	public ArrayList<OCLMatch> instantiate(Resource metamodelResource) {
		...
		while (eAllContents.hasNext()) {
			...
			if(eo instanceof EClass){
				for (EReference eref : ((EClass)eo).getEAllReferences()) {
					if(eref.isMany()){
						if(eref.getEReferenceType().equals(eo)){
							OCLMatch oclmatch = buildOCLMatch(((EClass)eo), eref);
	...
	private OCLMatch buildOCLMatch(EClass context, EReference eref) {
		String classA = context.getName();
		String attributeA = eref.getName();
		
		String expr = attributeA+""
				+ "->closure(iterator : "+classA+" | iterator."+attributeA+") "
				+ "->excludes(self)";
		OCLMatch res = new OCLMatch("context "+classA+" inv AcyclicReference: ", expr, context);
		res.setFeaturesInvolved(eref);
		return res;
	}
	
	@Override
	public String toString() {
		return "AcyclicReferenceOCLTemplate";
	}
}
