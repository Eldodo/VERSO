	...
	*/
	...
	public void render(GL gl) {
		...
		*/
		...
		int arraysSize = nbrePoints * this.renderingPoints.size();
		int arraysIndex = 0;
		
		float[] vertices = new float[arraysSize*3];
		float[] colors = new float[arraysSize*3];
		float[] normals = new float[arraysSize*3];
		short[] indices = new short[(arraysSize - 4) * 2 * 3];
		short[] currIndices = new short[nbrePoints];
		short[] nextIndices = new short[nbrePoints];
		short currIndex = 0;
		...
		while (renderingPointsItr.hasNext()) {
			...
			for (int i = 0; i < currentRenderingPoints.size(); i++) {
				vertex = currentRenderingPoints.get(i);			
				currNode = this.bSplinePoints.get(pointPos);
				
				n = new float[3];
				n[0] = vertex[0] - currNode[0];
				n[1] = vertex[1] - currNode[1];
				n[2] = vertex[2] - currNode[2];
				
				vertices[arraysIndex] = vertex[0];
				vertices[arraysIndex+1] = vertex[1];
				vertices[arraysIndex+2] = vertex[2];
				
				colors[arraysIndex] = currentPointColor.getRed()/255.0f;
				colors[arraysIndex+1] = currentPointColor.getGreen()/255.0f;
				colors[arraysIndex+2] = currentPointColor.getBlue()/255.0f;
				
				normals[arraysIndex] = n[0];
				normals[arraysIndex+1] = n[1];
				normals[arraysIndex+2] = n[2];
		...
		for (short i = 0; i < nbrePoints; i++) {
			currIndices[i] = i;
		}
		
		for (int i = 0; i < this.renderingPoints.size()-1; i++) {
			nextIndices = new short[nbrePoints];
			for (int k = 0; k < nbrePoints; k++) {
				nextIndices[k] = (short)(currIndices[k]+4);
			}
			
			for (int j = 0; j < currIndices.length; j++) {
				indices[currIndex++] = currIndices[j];
				indices[currIndex++] = nextIndices[(j + 1) % nextIndices.length]; 
				indices[currIndex++] = currIndices[(j + 1) % currIndices.length]; 
				
				indices[currIndex++] = currIndices[j];
				indices[currIndex++] = nextIndices[j];
				indices[currIndex++] = nextIndices[(j+1) % nextIndices.length]; 
			}
		...
		normalsBuffer = BufferUtil.newFloatBuffer(normals.length);
		colorsBuffer = BufferUtil.newFloatBuffer(colors.length);
		verticesBuffer = BufferUtil.newFloatBuffer(vertices.length);
		indicesBuffer = BufferUtil.newShortBuffer(indices.length);
		...
		gl.glDrawElements(GL.GL_TRIANGLES, indices.length, GL.GL_UNSIGNED_SHORT, indicesBuffer);		
