...
public class SystemRepresentation implements Renderable{	
	...
	protected void createLinks() {
		...
		System.out.println("\n\n***************************");
		System.out.println("(AVANT GC) Mémoire total de la heap: " + (s_Runtime.totalMemory() / (1024.0*1024.0)));
		System.out.println("(AVANT GC) Mémoire utilisé: " + ((s_Runtime.totalMemory() - s_Runtime.freeMemory()) / (1024.0*1024.0)));
		...
		System.out.println("\n(APRÈS GC) Mémoire total de la heap: " + (s_Runtime.totalMemory() / (1024.0*1024.0)));
		System.out.println("(APRÈS GC) Mémoire utilisé: " + ((s_Runtime.totalMemory() - s_Runtime.freeMemory()) / (1024.0*1024.0)));
		System.out.println("\n***************************");
	}
	...
	*/
	...
	public void render(GL gl)
	{
			...
			if (this.displayLinks) {
				...
				*/
				...
				if (this.dirtyLinks) {
					...
					System.out.println("(************************RENDER) Links displayed size: " + linksDisplayed.size());
					...
					*/
					...
					System.out.println("LINKS DISPLAYED SIZE / 2: " + (linksDisplayed.size() / 2));
					
					
					//for (LinkRepresentation link : linksDisplayed) {
					for (int i = 0; i < (linksDisplayed.size() / 2); i++) {	
					
					/*
						if (link.getStartNode().getEntityRepresentation().getName().equals("Class : org.jhotdraw.draw.BezierFigure") && link.getEndNode().getEntityRepresentation().getName().equals("Package : org.jhotdraw.draw.handle")) {
							System.out.println("Ok, passe!!!");
						}
						*/
						
						
						if (!(i >= linksDisplayed.size())) {
							linksDisplayed.get(i).render(gl);
						}
					...
					gl.glNewList(4, GL.GL_COMPILE);
						...
						for (int j = (linksDisplayed.size() / 2); j < linksDisplayed.size(); j++) {
		...
		int GL_GPU_MEM_INFO_TOTAL_AVAILABLE_MEM_NVX = 0x9048;
		int GL_GPU_MEM_INFO_CURRENT_AVAILABLE_MEM_NVX = 0x9049;

		IntBuffer totalMemAvailable = BufferUtil.newIntBuffer(1);
		IntBuffer currentMemAvailable = BufferUtil.newIntBuffer(1);
		
		
		gl.glGetIntegerv(GL_GPU_MEM_INFO_TOTAL_AVAILABLE_MEM_NVX, totalMemAvailable);
		gl.glGetIntegerv(GL_GPU_MEM_INFO_CURRENT_AVAILABLE_MEM_NVX, currentMemAvailable);

		totalMemAvailable.rewind();
		currentMemAvailable.rewind();
		
		System.out.println("MEMOIRE VIDEO TOTALE: " + totalMemAvailable.get());
		System.out.println("MEMOIRE VIDEO DISPONIBLE: " + currentMemAvailable.get());
	}
